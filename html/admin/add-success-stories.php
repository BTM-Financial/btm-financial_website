...
$editval = $db->getData("SELECT * FROM `success_stories` WHERE `id` = '$id'");
}else{
  $id = '';
}
...
if(isset($_REQUEST['submit'])){
    ...
    if(!empty($_FILES['image']['name'])){
		...
		$dest = "../uploads/success-stories/".$image;
	}else{
		$image = '';
	}

  // Slug query	
	$count = $db->getData("SELECT `id` FROM `success_stories` WHERE `title` = '".addslashes($title)."'");
	$RowId = $count['id'];
	if(empty($RowId)):
	...
	$result = $db->execute("INSERT INTO `success_stories`(`title`, `short_desc`, `description`, `image`, `slug`, `status`, `created_at`) VALUES ('$title', '$short_desc', '$description', '$image', '$slug', '$status', NOW())");
...
if(isset($_REQUEST['update'])){
	...
	{
	  unlink("../uploads/success-stories/".$oldfile);
	  $image = time()."-".$_FILES['image']['name'];
		$imagetmp = $_FILES['image']['tmp_name'];
		$dest = "../uploads/success-stories/".$image;
	} else {
  ...
  // Slug query	
	$count = $db->getData("SELECT `id` FROM `success_stories` WHERE `title` = '".addslashes($title)."' AND `id`!='$id'");
	$RowId = $count['id'];
	if(empty($RowId)):
		$slug=strtolower(trim(preg_replace("/[\s-]+/", "-", preg_replace( "/[^a-zA-Z0-9\-]/", '-', addslashes($title))),"-")); 
	else: 
	...
	$result = $db->execute("UPDATE `success_stories` SET `title` = '$title', `short_desc` = '$short_desc', `description` = '$description', `image` = '$image', `slug` = '$slug', `status` = '$status', `updated_at` = NOW() WHERE `id` = '$id'");
...
<head>
    ...
    <title><?= $websiteTitle; ?> | Add Success Stories</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Add Success Stories</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box box-warning">
            <div class="box-header with-border">
              ...
              <?php else: ?>
              <h3 class="box-title">Update Success Stories</h3>
              <?php endif; ?>
            </div>
            <div class="box-body">
                <?php if(isset($msg)){ ?>
              ...
              <form name="mybanner" id="mybanner" method="post" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                      <textarea name="short_desc" rows="5" cols="5" class="form-control"></textarea>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                      <textarea id="editor1" name="description" rows="20" cols="100"></textarea>
              ...
              <form name="mybanner" id="mybanner" method="post" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="inputExperience" class="col-sm-2 control-label">Short Description</label>
                    <div class="col-sm-6">
                      <textarea name="short_desc" rows="5" cols="5" class="form-control"><?php echo $editval['short_desc']; ?></textarea>
                ...
                <div class="form-group">
                    <label for="inputExperience" class="col-sm-2 control-label">Description</label>
                    <div class="col-sm-6">
                      <textarea id="editor1" name="description" rows="20" cols="100"><?php echo $editval['description']; ?></textarea>
                      <input name="imageEditor" type="file" id="upload" class="hidden" onChange="" style="display:none;">
                    </div>
                </div>
                ...
                <div class="form-group">
                  <label for="inputEmail" class="col-sm-2 control-label"></label>
                  <div class="col-sm-6">
                    <img src="../uploads/success-stories/<?php echo $editval['image']; ?>" id="uploadPreviews" style="width: 233px; height: 132px; border:1px solid #83888C; background-color: #ffffff;">
                  </div>
                </div>
