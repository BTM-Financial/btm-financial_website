...
$editval = $db->getData("SELECT * FROM `why_chooses` WHERE `id` = '$id'");
}else{
  $id = '';
}
...
if(isset($_REQUEST['submit'])){
    ...
    if(!empty($_FILES['image']['name'])){
		...
		$dest = "../uploads/why-chooses/".$image;
	}else{
		$image = '';
	}

  // Slug query	
	$count = $db->getData("SELECT `id` FROM `why_chooses` WHERE `title` = '".addslashes($title)."'");
	$RowId = $count['id'];
	if(empty($RowId)):
	...
	$result = $db->execute("INSERT INTO `why_chooses`(`title`, `description`, `image`, `slug`, `status`, `created_at`) VALUES ('$title', '$description', '$image', '$slug', '$status', NOW())");
...
if(isset($_REQUEST['update'])){
	...
	{
	  unlink("../uploads/why-chooses/".$oldfile);
	  $image = time()."-".$_FILES['image']['name'];
		$imagetmp = $_FILES['image']['tmp_name'];
		$dest = "../uploads/why-chooses/".$image;
	} else {
  ...
  // Slug query	
	$count = $db->getData("SELECT `id` FROM `why_chooses` WHERE `title` = '".addslashes($title)."' AND `id`!='$id'");
	$RowId = $count['id'];
	if(empty($RowId)):
		$slug=strtolower(trim(preg_replace("/[\s-]+/", "-", preg_replace( "/[^a-zA-Z0-9\-]/", '-', addslashes($title))),"-")); 
	else: 
	...
	$result = $db->execute("UPDATE `why_chooses` SET `title` = '$title', `description` = '$description', `image` = '$image', `slug` = '$slug', `status` = '$status', `updated_at` = NOW() WHERE `id` = '$id'");
...
<head>
    ...
    <title><?= $websiteTitle; ?> | Add Why Choose Us</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Add Why Choose Us</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box box-warning">
            <div class="box-header with-border">
              ...
              <?php else: ?>
              <h3 class="box-title">Update Why Choose Us</h3>
              <?php endif; ?>
            </div>
            <div class="box-body">
                <?php if(isset($msg)){ ?>
              ...
              <form name="mybanner" id="mybanner" method="post" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="inputExperience" class="col-sm-2 control-label">Description</label>
                    <div class="col-sm-6">
                    <textarea name="description" class="form-control" rows="10" cols="10"><?php echo $editval['description']; ?></textarea>  
                    </div>
                </div>
                ...
                <div class="form-group">
                  <label for="inputEmail" class="col-sm-2 control-label"></label>
                  <div class="col-sm-6">
                    <img src="../uploads/why-chooses/<?php echo $editval['image']; ?>" id="uploadPreviews" style="width: 233px; height: 132px; border:1px solid #83888C; background-color: #ffffff;">
                  </div>
                </div>
