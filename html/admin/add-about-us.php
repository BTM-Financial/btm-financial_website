...
$editval = $db->getData("SELECT * FROM `pages` WHERE `page_id` = '1'");
// update record query
if(isset($_REQUEST['update'])){
  ...
  if($_FILES['image']['name']!='')
	...
	$result = $db->execute("UPDATE `pages` SET `page_title` = '$page_title', `page_detail` = '$page_detail', `image` = '$image', `meta_title` = '$meta_title', `meta_description` = '$meta_description', `meta_keyword` = '$meta_keyword', `is_active` = '$status', `modified_by` = '".$_SESSION['ADMIN_USER_ID']."', `modified_at` = NOW() WHERE `page_id` = '1'");
...
<head>
    ...
    <title><?= $websiteTitle; ?> | Add about us</title>
