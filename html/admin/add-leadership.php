...
$editval = $db->getData("SELECT * FROM `leaderships` WHERE `id` = '$id'");
}else{
  $id = '';
}
...
if(isset($_REQUEST['submit'])){
    ...
    $designation = $db->addStr($_POST['designation']);
    $about = $db->addStr($_POST['about']);
	$status = $db->addStr($_POST['status']);

    if(!empty($_FILES['image']['name'])){
		$image = time()."-".$_FILES['image']['name'];
		$imagetmp = $_FILES['image']['tmp_name'];
		$dest = "../uploads/leaderships/".$image;
	}else{
		$image = '';
	}

 	$result = $db->execute("INSERT INTO `leaderships`(`name`, `designation`, `about`, `image`, `status`, `created_at`) VALUES ('$name', '$designation', '$about', '$image', '$status', NOW())");
...
if(isset($_REQUEST['update'])){
	...
	{
	  unlink("../uploads/leaderships/".$oldfile);
	  $image = time()."-".$_FILES['image']['name'];
	  $imagetmp = $_FILES['image']['tmp_name'];
	  $dest = "../uploads/leaderships/".$image;
	} else {
	...
	$result = $db->execute("UPDATE `leaderships` SET `name` = '$name', `designation` = '$designation', `about` = '$about', `image` = '$image', `status` = '$status', `updated_at` = NOW() WHERE `id` = '$id'");
...
<head>
    ...
    <title><?= $websiteTitle; ?> | Add Leadership</title>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Add Leadership</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box box-warning">
            <div class="box-header with-border">
              <?php if(empty($id)): ?>
              <h3 class="box-title">Add Leadership</h3>
              <?php else: ?>
              <h3 class="box-title">Update Leadership</h3>
              <?php endif; ?>
            </div>
            <div class="box-body">
                <?php if(isset($msg)){ ?>
              ...
              <form name="mybanner" id="mybanner" method="post" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label for="inputName" class="col-sm-2 control-label">Designation</label>
                  <div class="col-sm-6">
                    <input type="text" name="designation" value="<?php echo $editval['designation']; ?>" class="form-control" placeholder="Designation" required>
                  </div>
                </div>
                ...
                <div class="form-group">
                    <label for="inputExperience" class="col-sm-2 control-label">About</label>
                    <div class="col-sm-6">
                      <textarea name="about" class="form-control" rows="5"><?php echo $editval['about']; ?></textarea>
                    </div>
                </div>
                ...
                <div class="form-group">
                  <label for="inputEmail" class="col-sm-2 control-label"></label>
                  <div class="col-sm-6">
                    <img src="../uploads/leaderships/<?php echo $editval['image']; ?>" id="uploadPreviews" style="width: 233px; height: 132px; border:1px solid #83888C; background-color: #ffffff;">
                  </div>
                </div>
